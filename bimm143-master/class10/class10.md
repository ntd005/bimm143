...
First we need to important (i.e. read) our input data. I downloaded the
csv file from the class website:
...
and the funny last X column (col 35)
...
The 1st PC captures 44.27% of the original variance (look at cumulative
proportion). Note that 72.6% is captured in the first 3 PCs.
...
That is a hot mess\! We need to do out own PC1 vs PC2 plot and lets
color by the expert diagnosis.
...
To get our clusters out of this tree we need to CUT it with `cutree()`
function.
